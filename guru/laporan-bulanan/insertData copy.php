...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    foreach ($nama as $key => $value) {
        ...
        $ketuntasan_tartil = $ketuntasan_tartil[$key];
        $juz_clean = $juz[$key];
        $surat_clean = $surat[$key];
        $ketuntasan_tahfizh = $ketuntasan_tahfizh[$key];

        // Buat dan eksekusi query untuk memasukkan data ke dalam database
        $query = "INSERT INTO laporan (nama, bulan, jilid, halaman, ketuntasan_tartil, juz, surat, ketuntasan_tahfizh) VALUES ('$nama_clean', '$bulan_clean', '$jilid_clean', '$halaman_clean', '$ketuntasan_tartil', '$juz_clean', '$surat_clean', '$ketuntasan_tahfizh')";
